        ...
        "elements": {
            ...
            "items": {
                ...
                "properties": {
                    "type": {
                        "type": "string",
                        "enum": ["box", "circle", "diamond", "ellipse", "triangle", "text", "textblock", "line"],
                    },
...
2. Use `text` elements for labels that float near shapes; use `textblock` for wrapped prose or callouts with a panel background
...
def validate_spec(spec: Dict[str, Any]) -> list[str]:
    ...
    valid_types = {"box", "circle", "diamond", "ellipse", "triangle", "text", "textblock", "line"}
    ...
    for i, el in enumerate(spec["elements"]):
        ...
        if el_type in ("box", "circle", "diamond", "ellipse", "triangle"):
            ...
            if "x" not in el or "y" not in el:
                issues.append(f"Element {i} (textblock): missing x or y")
            for coord in ("w", "h"):
                if coord not in el:
                    issues.append(f"Element {i} (textblock): missing {coord}")
            if not el.get("text"):
                issues.append(f"Element {i} (textblock): missing text content")
